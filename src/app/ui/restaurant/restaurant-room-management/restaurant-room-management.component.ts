...
import { IcateGory, IItem } from "./roomList";
...
@Component({
  selector: "app-restaurant-room-management",
  templateUrl: "./restaurant-room-management.component.html",
  styleUrls: ["./restaurant-room-management.component.scss"],
})
export class RestaurantRoomManagementComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    console.log("tableDetails", this.tDetails);
    ...
    console.log("discountradiotype123", this.discountradiotype);
  ...
  getCategoryType() {
    this.roomManagementService.getCategoryMaster().subscribe((res) => {
      console.log("res: ", res);
  ...
  getTableInformation(invoiceId) {
    this.roomManagementService.getTableInfo(invoiceId).subscribe((res) => {
      console.log("res: ", res);
  ...
  getCategoryList(catid) {
    ...
    this.roomManagementService.getCategoryList(catid).subscribe((res) => {
      console.log("res: ", res);
  ...
  payMode() {
    this.roomManagementService.getAllPaymentMode().subscribe((res) => {
      console.log("res: ", res);
  ...
  getTax() {
    this.roomManagementService.getAllTaxType().subscribe((res) => {
      console.log("res: ", res);
    ...
    if (this.itemarr.length > 0) {
      for (var k in this.itemarr) {
        if (this.itemarr[k].itemId == dataitem.itemId) {
          ...
          this.totalPrice += +this.newrowdata.toFixed(2);
          this.sudototalPrice += this.newrowdata;
          this.textAmount += +this.newtax.toFixed(2);
        } else {
          console.log("pp", this.itemidarr.includes(dataitem.itemId));
          if (!this.itemidarr.includes(dataitem.itemId)) {
            ...
            this.totalPrice += +this.newrowdata.toFixed(2);
            this.sudototalPrice += +this.newrowdata.toFixed(2);
            this.textAmount += +this.newtax.toFixed(2);
          }
        }
      ...
      this.itemidarr.push(dataitem.itemId);
      this.totalPrice = dataitem.itemPrice.toFixed(2);
      this.sudototalPrice = dataitem.itemPrice;
      var taxvall = $("#taxId option:selected").text();
      this.textAmount = dataitem.itemTaxAmout;
      // this.textAmount = +this.textAmount.toFixed(2);
      this.grossAmount = +this.totalPrice + +this.textAmount.toFixed(2);
      this.grossAmount = +this.grossAmount.toFixed(2);
      console.log("totalPrice", this.totalPrice);
      console.log("textAmount", this.textAmount);
      console.log("taxvall", taxvall);
  ...
  roomSave() {
    ...
    if (this.totalPrice == undefined) {
    ...
    } else if (this.roomOrderData.persons == "") {
      this.alertSerive.create(
        "", //title
        "danger", //type
        20000, // time
    ...
    } else if (this.roomOrderData.waiter == "") {
      this.alertSerive.create(
        "", //title
        "danger", //type
        20000, // time
        "Please Enter Waiter Name!" //body
      );
    } else {
      console.log("fgfgfg===", this.itemarr);
      var orderArr = [];
      for (var k in this.itemarr) {
      ...
      var tabd = JSON.parse(this.tableDetails);
      ...
      this.roomOrderData.tableId = tabd.tableId;
      //  this.roomOrderData.discountType = this.discountradiotype;
      this.roomOrderData.typeValue = "save";
      this.roomOrderData.tableBookingId = this.tDetails.tableBookingId;
  ...
  backtable() {
    if (this.savedTableTransactionData == undefined) {
      this.roomManagementService
        .getTableBackStatus(
          this.residd,
          this.tDetails.tableId,
          this.tDetails.tableBookingId,
          this.tDetails.status
        )
        .subscribe((res) => {
          console.log("res: ", res);
