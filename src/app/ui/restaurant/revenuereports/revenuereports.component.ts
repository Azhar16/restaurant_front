...
import { RevenuereportsService } from "../../services/restaurant/revenuereports.service";
...
@Component({
  selector: 'app-revenuereports',
  templateUrl: './revenuereports.component.html',
  styleUrls: ['./revenuereports.component.scss']
})
export class RevenuereportsComponent implements OnInit {
  ...
  public currentDetailsPage: number;
   ...
   public revenueList:any;
   public revenueDetailsList:any;
 ...
 changeSearchFilter(filterText: string) {
   this.revenueList = this.rawBookingList.filter((bookingItem: any) =>
   bookingItem.restaurantName && bookingItem.restaurantName.toLowerCase().includes(filterText.toLowerCase())
       ...
       || bookingItem.tableOrder && bookingItem.tableOrder.toString().toLowerCase().includes(filterText.toLowerCase())
       || bookingItem.roomOrde && bookingItem.roomOrde.toString().toLowerCase().includes(filterText.toLowerCase())
       || bookingItem.percelOrder && bookingItem.percelOrder.toString().toLowerCase().includes(filterText.toLowerCase())
 ...
 pageChanged(event: PageChangedEvent): void {
   ...
   this.paginateBookingList = this.revenueList == null ? [] : this.revenueList.slice(startItem, endItem);
 }
 ...
 numPages(event: any) {
   ...
   restaurantMaster() {
     this.revenuereportsService.getRestaurantType().subscribe((res) => {
       console.log("res: ", res);
     ...
     if (this.revenueData.restaurantId == "") {
      ...
      console.log("revenueData", this.revenueData);
         ...
         this.revenuereportsService
           .getRevenueReportList(this.revenueData)
           .subscribe((res) => {
             console.log("revenueData", this.revenueData);
             ...
             if (this.revenueSaveData.status == "Success") {
               ...
               console.log("res: ", res);
                ...
                this.revenueList = [...this.rawBookingList];
                this.paginateBookingList = this.revenueList.slice(0, this.pageItems);
                }
                else{
                  this.revenueList =res["resObject"];
                }
                this.currentPage = 1;
                this.searchRequest = false;
   ...
   reportDetails(item){
    ...
    // localStorage.setItem("revenuedetailsdata", JSON.stringify(item));
    // this._des.newEvent("revenuereports_details");

    $('.revenue-report-details').show();
    $('.revenue-report').hide();
 console.log("date=",item)
//   item.transDate
// ).format("YYYY-MM-DD"));
...
this.revenuereportsService.getRevenueReportDetailsList(selectedDate,"2").subscribe((res) => {
  console.log("res: ", res);
  this.rawDetailsBookingList = res["resObject"];
  if(this.rawDetailsBookingList != undefined || this.rawDetailsBookingList != null){
  this.revenueDetailsList = [...this.rawDetailsBookingList];
  this.paginateDetailsBookingList = this.revenueDetailsList.slice(0, this.pageDetailsItems);
  }
  else{
    this.revenueDetailsList =res["resObject"];
  }
...
this.scrollBarContainerHeight =
  ...
  ($("#main-navbar").outerHeight() +
   ...
   changeDetailsSearchFilter(filterText: string) {
    this.revenueDetailsList = this.rawDetailsBookingList.filter((bookingItem: any) =>
    bookingItem.restaurantName && bookingItem.restaurantName.toLowerCase().includes(filterText.toLowerCase())
    || bookingItem.transDate && bookingItem.transDate.toLowerCase().includes(filterText.toLowerCase())
    || bookingItem.totalOrder && bookingItem.totalOrder.toString().toLowerCase().includes(filterText.toLowerCase())
    || bookingItem.invoiceNo && bookingItem.invoiceNo.toString().toLowerCase().includes(filterText.toLowerCase())
    || bookingItem.Order_Type_Name && bookingItem.Order_Type_Name.toString().toLowerCase().includes(filterText.toLowerCase())
    || bookingItem.totalRevenue && bookingItem.totalRevenue.toString().toLowerCase().includes(filterText.toLowerCase())
    || bookingItem.totalDiscount && bookingItem.totalDiscount.toString().toLowerCase().includes(filterText.toLowerCase())
    || bookingItem.totalTax && bookingItem.totalTax.toString().toLowerCase().includes(filterText.toLowerCase())
    || bookingItem.revenue && bookingItem.revenue.toString().toLowerCase().includes(filterText.toLowerCase())
  ...
  const startItem = (event.page - 1) * event.itemsPerPage;
  const endItem = event.page * event.itemsPerPage;
  this.paginateDetailsBookingList = this.revenueDetailsList == null ? [] : this.revenueDetailsList.slice(startItem, endItem);
  }
